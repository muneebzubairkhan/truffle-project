...
contract DigitalaxNFTStaking {
    ...
    */
    ...
    function updateReward(address _user) public {
        ...
        uint256 parentRewards = rewardsContract.parentRewards(
            lastUpdateTime,
            block.timestamp
        );

        if (stakedEthTotal > 0) {
            rewardsPerTokenPoints = rewardsPerTokenPoints.add(
                parentRewards.mul(1e18).mul(pointMultiplier).div(stakedEthTotal)
            );
        }
    ...
    function rewardsOwing(address _user) public view returns (uint256) {
        uint256 newRewardPerToken = rewardsPerTokenPoints.sub(
            stakers[_user].lastRewardPoints
        );
        uint256 rewards = stakers[_user]
            .balance
    ...
    function unclaimedRewards(address _user) external view returns (uint256) {
        ...
        uint256 rewards = stakers[_user]
            ...
            .div(pointMultiplier);
        return
            rewards.add(stakers[_user].rewardsEarned).sub(
                stakers[_user].rewardsReleased
            );
    }
    ...
    function claimReward(address _user) public {
        ...
        uint256 payableAmount = staker.rewardsEarned.sub(
